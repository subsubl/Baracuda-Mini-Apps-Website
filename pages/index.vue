...
const { data: apps, pending } = await useFetch('/api/apps', {
  // ⚡ Bolt Optimization: Pre-compute search strings once during fetch to avoid expensive string allocations
  // (toLowerCase) for every item on every keystroke/filter change in the computed property loop.
  transform: (appsData) => {
