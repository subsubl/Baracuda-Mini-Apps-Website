...
// Fetch apps data
// ⚡ Bolt Optimization: Pre-compute lowercased strings once during fetch to avoid O(N) string allocations during search filtering
...
const filteredApps = computed(() => {
  ...
  return apps.value.filter(app => {
    ...
    // Search check
    if (query) {
      // ⚡ Bolt Optimization: Use pre-computed strings instead of calling .toLowerCase() on every item
