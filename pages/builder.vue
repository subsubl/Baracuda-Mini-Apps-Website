...
const validateFiles = async () => {
    ...
    // ⚡ Bolt Optimization: Combine existence checks and file retrieval into a single pass
    // avoiding O(N) Array.from() allocations and multiple iterations over the Map.
    for (const [path, file] of filesMap.value.entries()) {
        ...
        if (hasAppInfo && hasIndexHtml) break; // Early exit
    }
...
const packApp = async () => {
    ...
    try {
        // ⚡ Bolt Optimization: Use iterator with early exit instead of Array.from().find()
