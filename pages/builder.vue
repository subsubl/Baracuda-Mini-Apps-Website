...
const validateFiles = async () => {
    ...
    // This avoids creating temporary arrays with Array.from(map.entries()) which creates O(N) allocation and garbage collection pressure.
    for (const [path, file] of filesMap.value.entries()) {
        ...
        if (hasAppInfo && hasIndexHtml) break;
    }
...
const packApp = async () => {
    ...
    try {
        // ⚡ Bolt Optimization: Replace Array.from(map.entries()).find() with a direct for...of loop with early exit.
        // Direct Map traversal avoids unnecessary O(N) array allocation.
