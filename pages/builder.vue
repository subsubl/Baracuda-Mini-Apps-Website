...
const validateFiles = async () => {
    ...
    // ⚡ Bolt Optimization: Combined validations and file lookup into a single pass
    // Removed O(N) Array.from() allocation and multiple loop passes
    for (const [path, file] of filesMap.value.entries()) {
        ...
        if (hasAppInfo && hasIndexHtml) break; // Early exit once both required files are found
    }
...
const packApp = async () => {
    ...
    try {
        // ⚡ Bolt Optimization: Replace O(N) Array.from allocation with direct iterator + early exit
