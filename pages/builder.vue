...
const validateFiles = async () => {
    ...
    // ⚡ Bolt Optimization: Single pass for validation and file extraction
    // Replaced Array.from().find() with a for...of loop to prevent unnecessary O(N) array allocation
    for (const [path, file] of filesMap.value.entries()) {
        ...
        if (hasAppInfo && hasIndexHtml) break; // Early exit when both files are found
    }
...
const packApp = async () => {
    ...
    try {
        // ⚡ Bolt Optimization: Replace Array.from().find() with for...of and early exit
        // Prevents unnecessary array allocation and stops iteration once the file is found
