...
const validateFiles = async () => {
    ...
    // ⚡ Bolt Optimization: Combined file lookups into a single for...of loop
    // Avoids O(N) Array.from allocation and repeated iterations over filesMap
    for (const [path, file] of filesMap.value.entries()) {
        ...
        // Early exit if we found both required files
...
const packApp = async () => {
    ...
    try {
        // ⚡ Bolt Optimization: Use for...of loop instead of Array.from to avoid O(N) memory allocation
