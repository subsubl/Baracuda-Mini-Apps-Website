...
## 2025-05-24 - [GraphQL Deep Recursive Search Bottleneck]
**Learning:** Fetching data via GitHub's GraphQL API using deep recursive search (like fetching `object { ... on Tree { entries { ... on Blob { text } } } }` for an entire directory tree) pulls all file content unconditionally. This causes excessive payload sizes and processing overhead when we only need specific files (like `appinfo.spixi`).
**Action:** Replace deep recursive tree fetching with a two-step approach: first list directories, then execute a batch query using index-based aliases (e.g., `app0_info`, `app0_tree`) explicitly mapped to the required targeted files.
