...
## 2025-06-11 - [Map Iteration Array.from Overhead]
**Learning:** Using `Array.from(map.entries()).find(...)` creates an intermediate array in memory, resulting in unnecessary $O(N)$ memory allocation and overhead. It also forces multiple passes when you might need to check for other files anyway.
**Action:** When performing case-insensitive lookups on a `Map` (especially when combining with other validations), use a single `for...of` loop over `entries()` to minimize iteration overhead and avoid garbage collection pressure.
