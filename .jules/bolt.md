...
## 2025-05-24 - [Avoid Array.from() with Map.entries() for lookups]
**Learning:** Using `Array.from(map.entries()).find()` for case-insensitive file lookups inside Nuxt/Vue components causes unnecessary O(N) array allocation overhead and garbage collection pressure, particularly when combined with repeated validations. Direct iteration over the map is approximately 3x faster and consumes far less memory.
**Action:** When performing lookups or validation against a `Map`, always use a `for...of` loop over `.entries()` or `.keys()`. Combine multiple checks into a single loop iteration with early exits wherever possible.
