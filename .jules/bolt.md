...
## 2025-04-27 - [Map Iteration Array.from Overhead]
**Learning:** Using `Array.from(map.entries()).find()` for lookups on Map structures (as seen in the browser-based Packer utility) causes unnecessary O(N) array allocation overhead and introduces an extra O(N) loop before the `.find()` executes. For small fixed size structures like hash arrays it is acceptable, but for dynamically sized structures like parsed directory trees it can impact performance and trigger garbage collection unnecessarily.
**Action:** When finding items in a Map structure without a direct `.get()` key lookup, use a `for...of` loop over `map.entries()` and implement early returns or `break` statements.
